...
import { MdDashboard, MdOutlineTransferWithinAStation, MdOutlineHome, MdOutlineGavel, MdShoppingCart, MdAttachMoney, MdCompareArrows } from "react-icons/md";
...
const Sidebar = () => {
    ...
    return (
        <div className={`bg-gradient-to-b from-[#67492b] to-[#f3bd85] text-white sticky top-0 h-screen flex flex-col transition-all duration-300 overflow-hidden ${sidebarOpen ? 'w-64' : 'w-20'} items-center lg:flex`}>
            <div className={`mb-8 pt-4 flex justify-center ${sidebarOpen ? 'w-full' : 'w-20'}`}>
                <img src={logo} alt="TeraLogo" className={`h-16 w-auto transition-all duration-300 ${sidebarOpen ? 'block' : 'hidden'}`} />
                {!sidebarOpen && <img src={logo} alt="TeraLogo Small" className="h-10 w-auto" />}
            </div>
            ...
            <button
                className={`flex items-center gap-4 text-white-500 p-3 rounded-full  mt-auto w-full justify-center lg:justify-start pl-4 lg:pl-6 transition-all duration-300 ${sidebarOpen ? 'block' : 'hidden lg:block'}`}
                onClick={handleLogout}
            >
                <FiLogOut className="text-xl" />
                <span className={`text-sm ${showSidebarText ? 'block' : 'hidden'}`}>Logout</span>
            </button>
        </div>
    );
};
...
const SidebarItem = ({ icon, label, active, onClick, showText }) => (
    <li
        onClick={onClick}
        className={`flex items-center gap-4 p-3 pl-4 lg:pl-6 rounded-tl-full rounded-bl-full cursor-pointer w-full ${active ? "bg-white text-[#003366]" : "hover:bg-[#1e3a8a]"}`}
    >
        {icon}
        <span className={`text-sm transition-all duration-300 ${showText ? 'block' : 'hidden'}`}>{label}</span>
    </li>
);
