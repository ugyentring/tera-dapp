...
function BuyLand() {
  ...
  const isFormValid = selectedBank && uploadedImage && journalNumber.trim() !== '';
  ...
  const allTransactions = Array.from({ length: 100 }, (_, index) => ({
    ...
    type: index % 3 === 0 ? "Buy" : index % 3 === 1 ? "Sell" : "Inheritance",
    ...
    documentUrl: "asdf",
    thramNo: "2A-234",
    imageUrl: `https://via.placeholder.com/300/${Math.floor(Math.random() * 16777215).toString(16)}/FFFFFF?Text=Land+Image`,
  }));
  ...
  <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center p-4 sm:p-6 overflow-y-auto">
    <div className="relative bg-white p-6 rounded-2xl shadow-2xl w-full max-w-4xl sm:w-11/12 md:w-10/12 lg:w-2/3 xl:w-1/2 max-h-[95vh] min-w-[60vw] overflow-y-auto">
      ...
      <h2 className="text-2xl font-semibold text-gray-800 mb-6 border-b pb-2">Transaction Overview</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <p><span className="font-semibold">Name:</span> {selectedTransaction.owner}</p>
        </div>

        {/* Divider */}
        <div className="col-span-2"><hr className="my-4 border-gray-300" /></div>
        ...
        <div>
          ...
          {selectedTransaction.buyerInfo ? (
            <>
              <p><span className="font-semibold">Name:</span> {selectedTransaction.buyerInfo.name}</p>
              <p><span className="font-semibold">Email:</span> {selectedTransaction.buyerInfo.email}</p>
            </>
          ) : (
        ...
        <div>
          ...
          {selectedTransaction.sellerInfo ? (
            <>
              <p><span className="font-semibold">Name:</span> {selectedTransaction.sellerInfo.name}</p>
              <p><span className="font-semibold">Email:</span> {selectedTransaction.sellerInfo.email}</p>
            </>
          ) : (
            <p className="text-gray-500">N/A</p>
          )}
        ...
        {/* Row 3 */}
        <div>
          <h3 className="text-lg font-semibold text-gray-700 mb-2">Land Information</h3>
          <p><span className="font-semibold">Land ID:</span> {selectedTransaction.land}</p>
          <p><span className="font-semibold">Location:</span> {selectedTransaction.landInfo.location}</p>
          <p><span className="font-semibold">Size:</span> {selectedTransaction.landInfo.size}</p>
        </div>
        ...
        <div>
          ...
          <ul className="list-disc pl-5 space-y-2 text-blue-600">
            <li>
              <a href={selectedTransaction.documentUrl1} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center">
                <AiOutlineDownload className="mr-2" />
                Document 1
              </a>
            </li>
            <li>
              <a href={selectedTransaction.documentUrl2} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center">
                <AiOutlineDownload className="mr-2" />
                Document 2
              </a>
            </li>
            <li>
              <a href={selectedTransaction.documentUrl3} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center">
                <AiOutlineDownload className="mr-2" />
                Document 3
              </a>
            </li>
...
{buyModalOpen && selectedTransaction && (
  <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center p-4 sm:p-6 overflow-y-auto">
    <div className="relative bg-white p-6 rounded-xl shadow-2xl w-full max-w-3xl max-h-[95vh] min-w-[60vw] overflow-y-auto">
      <h2 className="font-semibold text-xl text-gray-800 mb-6">Confirm Land Purchase</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          />
          <p><strong>Land ID:</strong> {selectedTransaction.land}</p>
          <div className="flex justify-between mb-4">
            <div>
              <p><strong>Owner:</strong> {selectedTransaction.owner}</p>
              <p><strong>Location:</strong> {selectedTransaction.landInfo.location}</p>
            </div>
            <div className="text-right">
              <p><strong>Size:</strong> {selectedTransaction.landInfo.size}</p>
              <p><strong>Thram No:</strong> {selectedTransaction.thramNo}</p>
            </div>
          </div>
        </div>
        ...
        <div>
          {/* Bank Dropdown */}
          <div className="mb-4">
            <label htmlFor="bank-select" className="block mb-2 font-semibold text-gray-700">
              Select Bank
            </label>
            <select
              id="bank-select"
          ...
          {/* Image Upload */}
          <div className="mb-4">
            <label htmlFor="image-upload" className="block mb-2 font-semibold text-gray-700">
              Upload Image
            </label>
            <input
              type="file"
          ...
          {/* Journal Number */}
          <div>
  <label htmlFor="journal-number" className="block mb-2 font-semibold text-gray-700">
    Journal Number
  </label>
  <input
    type="text"
      ...
      {purchaseSuccess && (
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 flex flex-col items-center space-y-6 relative animate-fadeIn">
            ...
            <h3 className="text-3xl font-semibold text-gray-900">Purchase Successful!</h3>
            <p className="text-gray-600 text-center max-w-xs">
              Your purchase has been completed successfully. Thank you for your business!
            </p>
            <button
        ...
        <div className="flex flex-col flex-1 p-4">
          ...
          <div className="flex flex-col sm:flex-row justify-between text-gray-600 text-sm gap-4 mb-4">
            <div>
              ...
              <p><strong>Location:</strong> {transaction.landInfo.location}</p>
            </div>
            <div className="sm:text-right">
              <p><strong>Size:</strong> {transaction.landInfo.size}</p>
              <p><strong>Thram No:</strong> {transaction.thramNo}</p>
            </div>
          </div>
